...
- mutate() creates new columns based on existing ones
- We use the `mutate()` function to create a new column `date_format`, using the `as.Date()` function to change the original `date` column's data type.
```{r}
#| warning: false
#| message: false
#| echo: true
...
- We can put all of these data transformation actions together using the `|>` function
  ...
  - line 2: creates formatted date column called `date_format` from the original `date` column.
...
- Starting with the `bracero` dataframe, code the following three actions using |> into a new dataframe called `bracero_ex1`: 
    - mutate the `date` column into a new column, called `date_format`. Change the data type from character to 'year-month-date'. 
...
1. Filter the `bracero_clean` dataframe for any interviews that took place before 2005 and with an interviewee whose name includes "Guadalupe".

2. Arrange the `bracero_clean` data frame in descending order by `date_format`. What was the date, how many interviews took place on that date, and who is listed first?
...
bracero_clean |>
 group_by(language) |>
 summarize(med_date = median(date_format))
```
