...
## Exercise four (collaborative)
1. Make the table pretty using the `gt` package. 
2. Create the second half of the table using the `county_vars_1930` dataframe for Edgecombe County, NC

## Rural Electrification Surveys{.smaller}
...
#| echo: true
#| eval: false
survey_edgecombe <- read_excel("survey_edgecombe.xlsx")
```

## Exercise Four (collaborative) {.smaller}
...
## Exercise Five (individual) {.smaller}
