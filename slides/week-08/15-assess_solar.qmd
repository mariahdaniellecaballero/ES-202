...
## Let's get Started
- Find your pair
- Seve and open the `HW_07` folder from [Google Drive](https://drive.google.com/drive/folders/12UpHYNcs2aEt4zmJxByd8ft2K932CS5r)
...
# | eval: true
# | echo: false
nn_sf <- read_sf("~/Documents/GitHub/ES-202/slides/week-08/data/nn_sf/nn_sf.shp")
energy_pts <- read_sf("~/Documents/GitHub/ES-202/slides/week-08/data/energy_pts/energy_pts.shp")
```
...
- `npv_rs_` is our x, perc_sf  is our y, and we'll choose fisher breaks for our style, and 3 colors for the dim option.
  - **NOTE**: Legend breaks vary by the story we’re trying to tell. There’s a handy book chapter that details the differences in legend classifications [here](https://geographicdata.science/book/notebooks/05_choropleth.html#).
...
- The `biscale` package uses the ggplot2 approach, which we’re already familiar with! 
...
I chose the “Bluegill” palette for this approach, but there are [several others](https://cran.r-project.org/web/packages/biscale/vignettes/bivariate_palettes.html) available.
```{r}
#| warning: false
#| message: false
#| echo: true
...
- I also want to create a nice legend to accompany this, so I’ll use the `bi_legend()` function to specify my palette, the number of colors, and the labels I’d like.
```{r}
#| warning: false
#| message: false
#| echo: true
