...
# | eval: true
# | echo: false
survey <- read_excel("/Users/mc152/Documents/GitHub/ES-202/slides/week-05/data/survey_edgecombe.xlsx", sheet = "Survey")

dictionary <- read_excel("/Users/mc152/Documents/GitHub/ES-202/slides/week-05/data/survey_edgecombe.xlsx", sheet = "Dictionary")
```
...
## ggplot{.smaller}
- ggplot is a package used to create really nice graphics. Plots work in layers, and each new layer should end with a + (similar to pipe)
```{r}
#| warning: false
#| message: false
#| echo: true
...
## Data Summaries 
- The `summary()` function is incredibly useful for viewing and understanding important aspects of your data. 
```{r}
#| echo: true
#| eval: true
summary(survey)
...
#| echo: true
survey |>
  select(RACE, TENURE, DAIRY_COW:HOGS) |>
  pivot_longer(
    cols = DAIRY_COW:HOGS,
    names_to = "animal",
    values_to = "count"
  ) |> 
  filter(RACE != "Unknown" & TENURE != "Unknown") |> 
  ggplot() + 
  geom_boxplot(aes(x = RACE, y = count, fill = TENURE)) + 
  scale_fill_manual(values = c("lightblue", "darkblue")) + 
  facet_wrap(~animal, scales = "free") + 
  theme_bw()
```

## Exercise 3 + 4 (collaborative){.smaller}
