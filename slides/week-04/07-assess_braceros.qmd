...
## Let's get Started
- Find your pair
- Download the [project file](https://drive.google.com/drive/folders/12UpHYNcs2aEt4zmJxByd8ft2K932CS5r) and unzip it in your Assignments folder.
...
# | eval: true
# | echo: false
bracero <- read_excel("/Users/mc152/Documents/GitHub/ES-202/slides/week-04/data/bracero_pull.xlsx")
```
...
- mutate() creates new columns based on existing ones
- We use the `mutate()` function to create a new column `year`, using the `year()` function to change the original `date` column to include only the year.
```{r}
#| warning: false
#| message: false
#| echo: true
...
- including `desc()` will sort a column from big to small. 
```{r}
#| warning: false
#| message: false
#| echo: true
...
bracero_clean |> 
  arrange(year, interviewee))
```

## Exercise Two {.smaller}
1. Filter the `bracero_ex1` dataframe for any interviews that took place before 2005 (using the `year` column) with an interviewee (`title`) whose name includes “Guadalupe”.
...
3. Filter the `bracero_clean` dataframe for any `description` values that mention “California”. How many interviewees are there?
...
## Logistics 
- No Class on Monday, Tuesday is Monday schedule! 
- Two readings on rural electrification in NC, one oral history
  - Ash leading centering activity :-)
